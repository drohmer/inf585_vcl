...
void initialize_density_visual(mesh_drawable& density_visual, size_t N)
{
	...
	density_visual = mesh_drawable(mesh_primitive_quadrangle({-1-dx/2,-1-dx/2,0},{1+dx/2,-1-dx/2,0},{1+dx/2,1+dx/2,0}, {-1-dx/2, 1+dx/2, 0}));
	
	density_visual.shading.phong = {1,0,0,1};
	density_visual.shading.color = {1,1,1};
	density_visual.shading.texture_inverse_y = true;
}

void initialize_grid(segments_drawable& grid_visual, size_t N)
{
...
void update_velocity_visual(segments_drawable& velocity_visual, buffer<vec3>& velocity_grid_data, grid_2D<vec2> const& velocity, float scale)
{
